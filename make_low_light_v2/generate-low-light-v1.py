...
def create_uneven_lighting(img: np.ndarray, multiply_range: tuple, add_range: tuple, vignetting_range: tuple) -> np.ndarray:
    """定制的非均匀光照，模拟随机光源和暗角"""
    h, w = img.shape[:2]
    dtype = img.dtype
    max_val = 65535.0 if dtype == np.uint16 else 255.0
    img_float = img.astype(np.float32)
    y, x = np.ogrid[:h, :w]
    light_x, light_y = np.random.uniform(w * 0.2, w * 0.8), np.random.uniform(h * 0.2, h * 0.8)
    dist_from_light = np.sqrt((x - light_x)**2 + (y - light_y)**2)
    max_dist = np.sqrt(w**2 + h**2)
    multiply_factor, add_factor = np.random.uniform(*multiply_range), np.random.uniform(*add_range)
    lighting_map = multiply_factor * (1 - dist_from_light / max_dist) + add_factor
    lighting_map = np.clip(lighting_map, 0.3, 1.5)
...
def adjust_color_temperature(img: np.ndarray, temp_type: str, warm_range: tuple, cool_range: tuple, intensity_range: tuple) -> np.ndarray:
    ...
    if temp_type == "warm" or (temp_type == "mixed" and np.random.random() < 0.5):
        temp_shift = np.random.uniform(*warm_range) / 6500
        color_matrix = np.array([1 + intensity * temp_shift, 1.0, 1 - intensity * temp_shift * 0.3])
    else:
        temp_shift = np.random.uniform(*cool_range) / 6500
        color_matrix = np.array([1 - intensity * 0.2, 1.0, 1 + intensity * (temp_shift - 0.5)])
    
    if len(img.shape) == 3:
        return np.clip(img_float * color_matrix, 0, max_val).astype(dtype)
    return img
...
def enhance_shadows(img: np.ndarray, strength_range: tuple) -> np.ndarray:
    ...
    if len(img.shape) == 3:
        gray = cv2.cvtColor((img_float * 255.0 / max_val).astype(np.uint8) if dtype == np.uint16 else img, cv2.COLOR_RGB2GRAY)
    else:
        gray = (img_float * 255.0 / max_val).astype(np.uint8) if dtype == np.uint16 else img
    ...
    return np.clip(img_float * shadow_factor, 0, max_val).astype(dtype)
...
def build_hybrid_augmentation_pipeline(cfg: dict) -> A.Compose:
    """
    构建一个混合了原生实现和Lambda函数的Albumentations流水线。
    这是最佳实践，兼顾了性能和效果的独特性。
    """
    aug_cfg = cfg["augmentation"]
    pipeline_steps = []

    # 为每个自定义函数创建一个接受 **kwargs 的包装器，以兼容多进程
    ...
    v_range = aug_cfg["uneven_lighting"].get("vignetting_strength", [0.1, 0.3])
    def create_uneven_lighting_wrapper(image, **kwargs):
        return create_uneven_lighting(image, multiply_range=aug_cfg["uneven_lighting"]["multiply_range"], add_range=aug_cfg["uneven_lighting"]["add_range"], vignetting_range=v_range)

    def add_iso_noise_wrapper(image, **kwargs):
        return add_iso_noise(image, intensity_range=aug_cfg["noise"]["iso_intensity"])

    def adjust_color_temperature_wrapper(image, **kwargs):
        return adjust_color_temperature(image, temp_type=aug_cfg["color_temperature"]["type"], warm_range=aug_cfg["color_temperature"]["warm_range"], cool_range=aug_cfg["color_temperature"]["cool_range"], intensity_range=aug_cfg["color_temperature"]["intensity_range"])

    def enhance_shadows_wrapper(image, **kwargs):
        return enhance_shadows(image, strength_range=aug_cfg["shadow_enhancement"]["strength_range"])


    # 1. 亮度、对比度、饱和度 (使用包装器以兼容多进程)
    ...
    # 2. 非均匀光照 (逻辑独特，使用包装器)
    pipeline_steps.append(A.Lambda(name="create_uneven_lighting", image=create_uneven_lighting_wrapper, p=aug_cfg["uneven_lighting"]["probability"]))

    # 3. 运动模糊 (使用原生实现，性能更好，代码更简洁)
    # 将sigma范围转换为Albumentations的blur_limit(核大小)
    ...
    # 4. ISO噪声, 色温, 阴影增强 (使用包装器)
    pipeline_steps.append(A.Lambda(name="add_iso_noise", image=add_iso_noise_wrapper, p=aug_cfg["noise"]["probability"]))
    pipeline_steps.append(A.Lambda(name="adjust_color_temperature", image=adjust_color_temperature_wrapper, p=aug_cfg["color_temperature"]["probability"]))
    pipeline_steps.append(A.Lambda(name="enhance_shadows", image=enhance_shadows_wrapper, p=aug_cfg["shadow_enhancement"]["probability"]))
...
def process_dataset(cfg: dict):
    ...
    print("增强流水线已由 Albumentations 构建 ('混合动力'模式)\n")
...
def main():
    ...
    parser.add_argument("--config", default="dark_config.yaml", help="YAML config path")
    args = parser.parse_args()

    cfg = load_config(args.config)
    print("=== 真实弱光环境图像生成器 ('混合动力' Albumentations版) ===")
